...
// Access mocked API functions from the actual imported module
  ...
  describe('inferAtlassianProduct', () => {
    ...
    describe('lookupJiraProjectType', () => {
      ...
      const setProjectType = (t: JiraProjectType) =>
        (apiClient.getJiraProjectTypeByKey as vi.Mock).mockResolvedValueOnce(t);
      ...
      it('caches cloud id + project type lookups', async () => {
        ...
        const cloudCalls = (apiClient.getCloudIDsForHostnames as vi.Mock).mock
          .calls;
        expect(cloudCalls.length).toBe(1);
        const projCalls = (apiClient.getJiraProjectTypeByKey as vi.Mock).mock
          .calls;
      ...
      it('logs and falls back when cloud id retrieval fails', async () => {
        __resetProductInferenceCaches();
        (apiClient.getCloudIDsForHostnames as vi.Mock).mockRejectedValueOnce(
      ...
      it('logs and falls back when project type lookup fails', async () => {
        __resetProductInferenceCaches();
        (apiClient.getCloudIDsForHostnames as vi.Mock).mockResolvedValueOnce({
          data: { tenantContexts: [{ cloudId: 'cloud-2' }] },
        });
        (apiClient.getJiraProjectTypeByKey as vi.Mock).mockRejectedValueOnce(
