 ...
 */
function isCompassScorecardNotification(notification: AtlassifyNotification): boolean {
  return notification.message.includes('a scorecard');
}

class CompassStrategy extends DefaultStrategy {
  override avatarAppearance(notification: AtlassifyNotification): 'circle' | 'square' {
