...
4. **Milestone lifecycle is automated.** An open `Release X.Y.Z` [Milestone][github-milestones] is created from the release-please version and merged pull requests are attached to it as they land. Once the release is published, the milestone is finalized with the release URL and date, and a separate job comments once on each included pull request with a link to the release. Notification retries are idempotent. No manual milestone creation or renaming is required.
